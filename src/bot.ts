...
class TranscriptionBot {
  ...
  try {
    ...
    if (message.type === "transcript") {
      ...
      const utteranceText = transcriptionData.utterance.text || "";

      // Print transcription with meeting URL if available
      const meetingUrl = transcriptionData.meetingUrl || "UnknownMeeting";
      

      logger.info(
        `[${meetingUrl}] Transcription ${isPartial ? "(partial)" : "(final)"}: ${utteranceText}`
      );
